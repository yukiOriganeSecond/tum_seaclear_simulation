function plotRobotStatesErrorFB(q,q_nominal,q_nonFB,param,t_vec,dimset,folder_name,sum_set)
...
%   詳細説明をここに記述
    ...
    ylabset = ["$\theta^r(t)-\theta(t)$ (rad)","$\dot{\theta}^r(t)-\dot{\theta}(t)$ (rad/s)","$l^r(t)-l(t)$ (m)","$\dot{l}(t)^r-\dot{l}(t)$ (m/s)","$X(t)^r-X(t)$ (m)","$\dot{X}(t)^r-\dot{X}(t)$ (m/s)","$r^r(t)-r(t)$ (m)","$\dot{r}^r(t)-\dot{l}(t)$ (m/s)"];
    ...
    for m = 1:size(dimset,1)
        for n = 1:size(dimset,2)
            ...
            if length(sum_set) == 1
                ...
                hold on
                for s = sum_set
                    plot(t_vec, q_nominal(dimset(m,n),:,1)-q(dimset(m,n),:,s),'Color','b','LineWidth',0.8);
                    plot(t_vec, q_nominal(dimset(m,n),:,1)-q_nonFB(dimset(m,n),:,s),'Color','g','LineWidth',0.8);
                    %if (ismember(dimset(m,n),[7,8]))    % also show l,l_dot when show r, r_dot
