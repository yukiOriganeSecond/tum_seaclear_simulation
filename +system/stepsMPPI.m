...
function [q,f,u,face_infeasible,F_] = stepsMPPI(param_sim, param_nominal, param_plan_list, W_sim, W_plan_list)   % simulation loop
    ...
    for t_sys = 1:param_nominal.Nt-1    % system loop
        if (mod(t_sys,param_nominal.input_prescale) == 1) || (t_sys == 1) || (param_nominal.input_prescale == 1)
            ...
            if param_nominal.visual_capture
                ...
                [us_pred, F_(j)] = system.controllerMPPI(q(:,t_sys), f(:,t_sys), us_pred, param_nominal, param_plan_list, W_plan_list);
            else
                [us_pred, ~] = system.controllerMPPI(q(:,t_sys), f(:,t_sys), us_pred, param_nominal, param_plan_list, W_plan_list);
            end
            u(:,t_sys) = us_pred(:,1);
    ...
    face_infeasible = 0;    % DEBUG CODE HERE!!
end