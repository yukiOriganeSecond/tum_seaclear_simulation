...
%%
simulation_folder_name = "multiple_0308_without_Xd";
...
for sim_index = 1:length(each_simulation_folder_list)
    ...
    parfor result_index = 1:length(each_result_file_list)
        disp("processing... simulation: "+each_simulation_folder_list(sim_index).name+", result: "+each_result_file_list(result_index).name)
        vals_ = load(simulation_folder_each+"/variables/"+each_result_file_list(result_index).name, "u","f","q","x","param_nominal","s","seed_simulate");
        param_nominal = vals_.param_nominal;
        ...
        visual.plotInputs(vals_.u,vals_.f,param_nominal,t_vec,[1,2;3,4],simulation_folder_each+"/inputs/"+name_without_extension+"_",1:length(vals_.seed_simulate),visialize_graphics)
        visual.makeSnapsWithPoints(vals_.q,vals_.x,param_nominal,scenario(vals_.s),t_vec,simulation_folder_each+"/paths/"+name_without_extension+"_",[1],1:length(vals_.seed_simulate),visialize_graphics);
    end
