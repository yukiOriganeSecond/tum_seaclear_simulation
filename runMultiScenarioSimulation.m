...
function runMultiScenarioSimulation(method_container, scenario_name, simulation_name, Nsim, Nplan, param_error, robot_mass, kill_all_visualize)
    ...
    for s = 1:Nsc % loop for scenario
        ...
        for method_index = 1:Nm
            ...
            param_base = system.addParam(param_base,"obs_pos",scenario(s).obs_pos,"White_Const",0.30*ones(size(scenario(s).obs_pos)));
            param_base = system.addParam(param_base,"obs_size",scenario(s).obs_size,"White",0.1*ones(size(scenario(s).obs_size)));
