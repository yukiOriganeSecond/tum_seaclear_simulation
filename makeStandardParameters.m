...
function param_base = makeStandardParameters(method)
    ...
    if ismember(method,["RA-SAA","RA-SAA-PID"])
        ...
        param_base = system.addParam(param_base,"opt_Display",'iter',"Deterministic");
        param_base = system.addParam(param_base,"opt_PlotFcn",'optimplotfvalconstr',"Deterministic");
        if method == "RA-SAA"
    ...
    elseif method == "MPPI"
        ...
        % Optimize Weight Matrix
        param_base = system.addParam(param_base,"Q",diag([100,100,100,100,100,100]),"Deterministic");   % cost matrix for state (x, d)
        param_base = system.addParam(param_base,"R",diag([1, 1, 1, 1])./(param_base.m.average^2),"Deterministic");   % cost matrix for input (u_theta, u_r, U_l, U_X)  
        param_base = system.addParam(param_base,"P",diag([10000,10000,10000,10000,10000,10000]),"Deterministic");
        % initial solution
